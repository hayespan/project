...
var $counter, _, common, countdown, humanize, jquery;
...
window.onload = function() {
  common.init();
  return _.each($counter, function(counter) {
    var interval;
    interval = 1000;
...
countdown = function(counter) {
  var counterTime, currentTime, passedTime, releasedTime, timeDelta;
  ...
  if (counterTime < 0) {
    jquery(counter).prev().text('已超时').css({
      color: "red",
      fontSize: '20px'
    });
    return clearTimeout(counter.interval);
  } else {
    return jquery(counter).text(humanize(counterTime, {
      language: "zh-CN",
      delimiter: "",
      spacer: "",
      round: true,
...
},{"./common.coffee":4,"humanize-duration":1,"jquery":2,"lodash":3}]},{},[5]);
